...
def generate_response(
    ...
    if condition in [
        ...
        # Get the indices not in parallel_ordering
        # E.g. if there are 5 parallel substrings and parallel_orderings is [0,1,2,3], then remaining_indices is [4] so we don't drop the 5'th substring
        all_indices = set(range(len(parallel_substrings)))
        used_indices = set(parallel_ordering)
        remaining_indices = list(all_indices - used_indices)
        
        # Create full ordering: specified permutation followed by remaining indices
        full_ordering = list(parallel_ordering) + remaining_indices
        
        # Use full ordering to get all substrings
        substrings = list(np.asarray(parallel_substrings)[full_ordering])
    else:
