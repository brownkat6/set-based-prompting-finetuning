...
def get_tokenized_input_prompt(
    ...
    for tokOption in tokParallel:
        tokAll["input_ids"] = torch.cat(
            (tokAll["input_ids"], tokOption["input_ids"][0].unsqueeze(0)), dim=1  # type: ignore
        )  # type: ignore
    ...
    tokAll["input_ids"] = torch.cat(
        (tokAll["input_ids"], tokD["input_ids"][0].unsqueeze(0)), dim=1  # type: ignore
    )  # type: ignore
    tokAll["attention_mask"] = torch.cat(
        (tokAll["attention_mask"], tokD["attention_mask"][0].unsqueeze(0)), dim=1  # type: ignore
    )  # type: ignore
    return tokAll
...
def get_tokenized_input_prompt_recursive(
    ...
    '''
    tokAll = {'input_ids':torch.zeros((1,0),dtype=torch.int),'attention_mask':torch.zeros((1,0),dtype=torch.int)}
    for toks in tokSequences:
        if not isinstance(toks,list):
            tokAll["input_ids"] = torch.cat(
                (tokAll["input_ids"], toks["input_ids"][0].unsqueeze(0)), dim=1  # type: ignore
            )  # type: ignore
            tokAll["attention_mask"] = torch.cat(
...
def genOrderIndependentOutputRecursive(
    ...
    tokenizer: typing.Union[transformers.PreTrainedTokenizer, transformers.PreTrainedTokenizerFast],
    ...
    for i,seq in enumerate(inputStrings):
        if isinstance(seq,list):
            if reverse_parallel_substrings_order:
                seq=seq[::-1]
            tokSequences.append([tokenizer(input_text, return_tensors="pt", add_special_tokens=(i==0)).to(torch_device) for input_text in seq])
        else:
            tokSequences.append(tokenizer(seq,return_tensors="pt",add_special_tokens=(i==0)).to(torch_device))
    ...
    if not is_order_independent:
        ...
        position_ids = get_position_ids_nopad_recursive(
            tokSequences
        )
        ...
        attention_mask_2d = get_attention_mask_2d_recursive(
            tokSequences, tokAll
        ).to(torch_device)
...
def calc_perplexity(input_ids, attention_mask, position_ids, model):
    ...
    out=model(input_ids, attention_mask=attention_mask, position_ids=position_ids, labels=input_ids, return_dict=True)
    loss=out.loss.item()
...
def calc_options_perplexity_dict(input_ids,attention_mask,position_ids,metadata,model,tokenizer):
    ...
    prompt_loss,prompt_perplexity = calc_perplexity(input_ids, attention_mask, position_ids, model)
    option_perplexities = {"prompt":prompt_perplexity}
    for option in [label]+incorrect_answers:
        option_perplexity = calc_perplexity_option(input_ids,attention_mask,position_ids,option,model,tokenizer)
...
def order_independent_query(
    ...
    assert isinstance(generated,transformers.utils.ModelOutput), f"generated={generated} is not a ModelOutput, but a {type(generated)}"
    text = tokenizer.decode(generated.sequences[0], skip_special_tokens=True)[
        inputPromptLen:
    ]
    ...
    if metadata is not None and "label" in metadata and "incorrect_answers" in metadata:
        ...
        class_ids = torch.LongTensor(tokenizer([metadata["label"]]+metadata["incorrect_answers"], padding=True).input_ids)
        ...
        label_scores = {k:v for k,v in zip([metadata["label"]]+metadata["incorrect_answers"],list(mean_logit_scores_per_label[0].numpy()))}
...
def record_accuracy(output_file_name:str,accuracy_file:str, accuracy_scoring_mode:typing.Union[typing.Literal["pct_raw_output_contains_correct_answer_only"], typing.Literal["max_token_prob"]]) -> None:
    '''
    ...
    '''
    assert(accuracy_scoring_mode in ["pct_raw_output_contains_correct_answer_only","max_token_prob"])
    accuracy_key = "raw_output_contains_correct_answer_only" if accuracy_scoring_mode=="pct_raw_output_contains_correct_answer_only" else "is_correct_answer"
    df=pd.DataFrame(columns=["model_name","max_new_tokens","output_filename","output_type","accuracy","n_samples","accuracy_scoring_mode"])
        ...
        for model_name in np.unique([r["model"] for r in records]):
            ...
            for i,r in enumerate(records):
                if accuracy_key not in r["responses"][output_type]:
                    print(f"{accuracy_key} not in row {i} with keys",r["responses"][output_type].keys())
            accuracy = np.mean([r["responses"][output_type][accuracy_key] for r in records if r["model"]==model_name])
            pd.DataFrame.from_dict({
                ...
                "accuracy":[accuracy],
                "n_samples":[len([r for r in records if r["model"]==model_name])],
